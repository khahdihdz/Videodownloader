...
class DownloadService : Service() {
 ...
 private lateinit var d: Downloader
 override fun onCreate(){super.onCreate();d=Downloader(this);if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){getSystemService(NotificationManager::class.java).createNotificationChannel(NotificationChannel("dl","Downloads",NotificationManager.IMPORTANCE_LOW))};startForeground(1,n("Đang chuẩn bị",0))}
 override fun onStartCommand(i:Intent?,flags:Int,id:Int):Int{
  ...
  scope.launch{try{val info=VideoInfo(url,UrlDetector.detect(url),title,0L,null,emptyList());val f=Format(fid,h,if(h>0) h.toString()+"p" else "Tốt nhất");val tmp=File(cacheDir,"video_"+System.currentTimeMillis()+".mp4");d.download(info,f,tmp).collect{p->startForeground(1,n("Đang tải: "+title,p))};save(tmp,title);tmp.delete();startForeground(1,n("Đã tải xong",100));delay(1200)}catch(_:Throwable){startForeground(1,n("Tải video thất bại",0));delay(1200)}finally{stopSelf(id)}};return START_NOT_STICKY}
