...
class Downloader(context: Context) {
 ...
 private fun fetchOembedOrPage(url:String):Triple<String,Long,String?> {
  ...
  return try {
   ...
   conn.connectTimeout=10000;conn.readTimeout=10000;conn.setRequestProperty("User-Agent","Mozilla/5.0")
   val html=conn.inputStream.bufferedReader().use{it.readText()}
   val title=Regex("""<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']""",RegexOption.IGNORE_CASE).find(html)?.groupValues?.get(1)
    ?: Regex("""<title>(.*?)</title>""",RegexOption.IGNORE_CASE).find(html)?.groupValues?.get(1) ?: "Video"
   val thumb=Regex("""<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']""",RegexOption.IGNORE_CASE).find(html)?.groupValues?.get(1)
   Triple(title.replace("&amp;","&"),0L,thumb)
  } catch(e:Throwable) { throw IllegalStateException("Không lấy được metadata: "+(e.message?:"unknown"),e) }
 }
