...
@Composable fun App(vm:MainVM=viewModel()){
 ...
 info?.let{x->AsyncImage(x.thumbnail,null,Modifier.fillMaxWidth().height(190.dp));Text(x.title,style=MaterialTheme.typography.titleLarge);Text(x.platform.name);Text("Chất lượng");x.formats.forEach{f->Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Text(f.label);Button({vm.download(x,f)}){Text("Tải")}}}}
 }
}